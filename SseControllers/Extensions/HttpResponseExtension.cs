...
internal static class HttpResponseExtension
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await response.WriteSseEventFieldAsync(SseConstants.SseIdField, clientEvent.Id, cancellationToken);
        }

        if (!string.IsNullOrWhiteSpace(clientEvent.Type))
        {
            await response.WriteSseEventFieldAsync(SseConstants.SseEventField, clientEvent.Type, cancellationToken);
        }

        foreach (string line in clientEvent.Data)
        {
            await response.WriteSseEventFieldAsync(SseConstants.SseDataField, line, cancellationToken);
        }
